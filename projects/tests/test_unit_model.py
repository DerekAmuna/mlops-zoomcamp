...
def test_model_service_prepare_features():
    ...
    assert features['fixed_acidity'] == sample_data['fixed_acidity']
...
def test_model_service_lambda_handler():
    ...
    encoded_data = base64.b64encode(json.dumps(sample_event).encode('utf-8')).decode('utf-8')
    print(encoded_data)
    event = {
        'Records': [{
            'kinesis': {
